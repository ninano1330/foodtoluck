		...
		<div class="row">	
			...
			<div class="col-lg-9">
				...
				<div class="card card-outline-secondary my-4">
					<div class="card-header">트럭 수정</div>
					<div class="card-body">
						<form action="modifyTruck" method="post" enctype="multipart/form-data">
							...
							<table class="tbl_fieldset">
								...
								<tbody>
									<tr>
										<th>트럭 이름</th>
										<td>
											<input type="text" id="truck_name" name="truck_name" value='${foodtruck.truck_name }'>
										</td>
									</tr>
									<tr>
										<th>트럭 소개</th>
										<td>
											<textarea row="20" cols="50" name="truck_intro">${foodtruck.truck_intro }</textarea>
										</td>
									</tr>
									<tr>
										<th>트럭 음식 분류</th>
										<td>
											...
											<%-- <c:forEach items="${allTypelist}" var="foodlist">
												<c:forEach items="${typelist }" var="ty">
													<c:choose>
														<c:when test="${foodlist.foodtype_name==ty }">
															<input type="checkbox" id="MenuCheckbox" name="MenuCheckbox" value="${foodlist.foodtype_code}" checked> ${foodlist.foodtype_name}
														</c:when>
														<c:otherwise>
											...
											<c:forEach items="${allTypelist}" var="foodlist">
											<input type="checkbox" id="MenuCheckbox" name="MenuCheckbox" value="${foodlist.foodtype_code}"
												<c:forEach items="${typelist }" var="ty">
													<c:choose>
														<c:when test="${foodlist.foodtype_name==ty }">
														checked
